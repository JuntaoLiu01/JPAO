...
model_dir = os.path.join(base_dir,"models/uncased_L-12_H-768_A-12")
MAX_SEQ_LENGTH = 10
...
flags.DEFINE_string("vocab_file", os.path.join(model_dir,"vocab.txt"),
...
def parse_res(result, all_tokens):
  # tf.logging.info("***** Predict results *****")
  i = 0
  sentences = []
  for word_loss in result:
    ...
    if all_tokens[i] == "[SEP]":
      sentence["tokens"] = tokens
      sentence["ppl"] = float(sentence_loss / word_count_per_sent)
      sentences.append(sentence)
      i += 1
  return sentences
...
def bert_ppl(sentences):
  ...
  result = estimator.predict(input_fn=predict_input_fn)
  res = parse_res(result,all_tokens)
  return res

estimator = build_estimator()
if __name__ == "__main__":
  res = bert_ppl(["beautiful fabric","beautiful silk fabric"])
  print(res)
